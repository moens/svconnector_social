...
//					$GLOBALS['TYPO3_CONF_VARS']['SYS']['caching']['cacheConfigurations'][$this->extKey]['frontend'],
	...
	public function handleArray($rawData, $tableTCA, $index, $externalConfig, $numAdditionalFields, $additionalFields) {
		...
		if (is_array($rawData) && count($rawData) > 0) {
			if (TYPO3_DLOG) t3lib_div::devLog('Input from the fx(call)', $this->extKey, 1, $rawData);
			...
			$records = jsonPath($rawData, "$." . $externalConfig['nodetype'] . "[*]");
			if (TYPO3_DLOG) t3lib_div::devLog('data after jsonpath', $this->extKey, 1, $records);
			if(count($records) > 0) {
